def initialize_search(start, end, graph):
    ...
    F = {start: graph.heuristic(start, end)}  # Estimated movement cost of start to end going via this position
    closed_vertices = set()
...
def a_star_search(start, end, graph):
    G, F, closed_vertices, open_vertices, came_from = initialize_search(start, end, graph)
